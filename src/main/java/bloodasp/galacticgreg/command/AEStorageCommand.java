					...
					for (int lX = 1; lX <= storedSize.x; lX++) {
						for (int lY = 65; lY < 65 + storedSize.y; lY++) {
							for (int lZ = 1; lZ <= storedSize.z; lZ++)  {
								...
								NBTTagCompound tTECompound = null;
								...
								if (b != null)
								{
									...
									if (bTE != null)
									{
										tTECompound = new NBTTagCompound();
										bTE.writeToNBT(tTECompound);

										// Print a warning on the console
										tMsg += " TE";
										GalacticGreg.Logger.warn("Warning: Found TileEntity at X[%d] Y[%d] Z[%d]. Handling those is experimental!", lX, lY, lZ);
										if (!tTEWarningSend)
										{
									...
									if (b != Blocks.air)
										tSchematic.addStructureInfo(SpaceSchematicFactory.createStructureInfo(lX, lY, lZ, b, bm, tTECompound));
								}
							}
						}
					}
